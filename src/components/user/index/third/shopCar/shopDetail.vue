<template>
  <main id="shopDetail">
    ...
    <img v-for="(itemD,index) in detailPicList" :key="index" :src="itemD" alt="" class="PicList" lazy>
		...
		<!-- 购买按钮 -->
		<div class="BuyBtns">
      <svg @click="addCollection" class="icon BuyBtnsLeft"  v-bind:class="{active: isClick==true}" aria-hidden="true">
        <use xlink:href="#icon-shoucangjia"></use>
      </svg>
			<div class="BuyBtnsRight">
        <el-button type="warning" round @click="addShopCar" :plain="true">加入购物车</el-button>
        <el-button type="danger" round @click="nowBuy">立即购买</el-button>
			</div>
		</div>
  </main>
</template>
...
<script>
  ...
  import { getOneShopMsg, getOneShopDetail, getOneShopPhoto, checkShopCollection, AddcolShopCar, AddShopCar, AddShopAccounts } from '../../../../../../server/api/user.js'
  ...
  export default{
    data(){
      return{
        ...
        oldPrice: "",
        // id
        message: "#/shopMsg/"+this.$route.params.id,
      }
    },
    methods:{
      getOneShopMsg(){
        getOneShopMsg(getCookie('userID'), this.$route.params.id).then((res) => {
          ...
          // 收藏夹验证
          checkShopCollection(getCookie('userID'), this.$route.params.id).then((res) => {
            if(res.length==0){
              this.isClick = false
            }else{
              this.isClick = true
      ...
      // 获取轮播图
      getOneShopCau(){
        getOneShopPhoto(getCookie('userID'), this.$route.params.id).then((res) => {
          this.picList = res
        }).catch(err => {
          console.log(err)
        })
      ...
      // 获取详情图片列表
      getOneShopPic(){
        getOneShopDetail(getCookie('userID'), this.$route.params.id).then((res) => {
      ...
      // 加入购物车
      addShopCar(){
        AddShopCar(getCookie('userID'), this.$route.params.id).then((res) => {
          Toast({
            message: '添加成功',
            position: 'middle',
            duration: 1000,
      ...
      // 立即购买
      nowBuy(){
        AddShopAccounts(getCookie('userID'), this.$route.params.id, 1).then((res) => {
      ...
      addCollection(){
        if(this.isClick==false){
          AddcolShopCar(getCookie('userID'), this.$route.params.id).then((res) => {
            this.isClick = true
          }).catch(err => {
            console.log(err)
          })
